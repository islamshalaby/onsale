...
//        }
    ...
    public function payments_date(Request $request)
    {
        ...
        if (count($data) > 0) {
            ...
            if ($lang == 'ar') {
                $data['message'] = $setting->payment_hint_ar;
            } else {
                $data['message'] = $setting->payment_hint_en;
            }
            $response = APIHelpers::createApiResponse('custom', 200, $setting->payment_hint_en, $setting->payment_hint_ar, (object)[], $request->lang);
            return response()->json($response, 200);
        }
